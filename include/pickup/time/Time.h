...
class Time {
 public:
   ...
   *   要创建一个表示当前时间的 Time 对象，请使用 getCurrentTime()。
   *  @see getCurrentTime
   */
  Time() = default;

  /** 基于毫秒数创建一个时间。
   *    要创建一个设置为当前时间的 Time 对象，请使用 getCurrentTime()。
   *
  ...
  /** 返回毫秒数，0 到 999。
   *
   *    与 toMilliseconds() 不同，此方法只返回当前秒内的毫秒数，
   *    而不是自纪元以来的总毫秒数。
   *
   *     @see toMilliseconds
   */
  ...
  */
  ...
  /** 将 Timespan 添加到此时间。 */  // **将声明移到类的 public 部分**
  Time& operator+=(Timespan delta) noexcept;
  /** 从此时间中减去 Timespan。 */  // **将声明移到类的 public 部分**
  Time& operator-=(Timespan delta) noexcept;
  ...
  */
  ...
  /** 返回自固定事件（通常是系统启动）以来的毫秒数。
      ...
      此方法与 getMillisecondCounter() 功能相同，但返回更精确的值，
      如果可用则使用更高分辨率的计时器。
 ...
 private:
  ...
  static std::atomic<uint32_t> lastMSCounterValue_;  // **现在有了 <atomic> 头文件**
};
